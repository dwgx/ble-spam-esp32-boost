...
// ESP32 Boost (dual-radio via Ghost ESP UART) by @dwgx
...
// Map current attack's protocol to ESP32 Ghost ESP command
static EspBoostCmd get_esp_boost_cmd(const Attack* attack) {
    if(!attack || !attack->protocol) return EspBoostCmdRandom;
    const Protocol* p = attack->protocol;
    if(p == &protocol_continuity) return EspBoostCmdApple;
    if(p == &protocol_easysetup) return EspBoostCmdSamsung;
    if(p == &protocol_fastpair) return EspBoostCmdGoogle;
    if(p == &protocol_swiftpair) return EspBoostCmdWindows;
...
static void toggle_adv(State* state) {
    if(state->advertising) {
        ...
        // Stop ESP32 boost
        esp_boost_stop(state->ctx.esp_boost);
    } else {
        state->advertising = true;
        furi_thread_start(state->thread);
        // Start ESP32 boost with matching attack type
        if(state->index >= 0 && state->index < ATTACKS_COUNT) {
            esp_boost_start(state->ctx.esp_boost, get_esp_boost_cmd(&attacks[state->index]));
        }
    }
}
...
static void draw_callback(Canvas* canvas, void* _ctx) {
    ...
    if(esp_boost_is_connected(state->ctx.esp_boost)) {
        canvas_draw_str(canvas, 14, 12, "BLE Spam+ESP");
    } else {
    ...
    case PageAboutCredits:
        ...
        elements_text_box(
            ...
            "ESP32 Boost: \e#dwgx\e#  " FAP_VERSION,
            false);
        break;
    default: {
        if(!attack) break;
...
int32_t ble_spam(void* p) {
    ...
    state->ctx.esp_boost = esp_boost_init(); // NULL if no ESP32, that's fine
    ...
    esp_boost_free(state->ctx.esp_boost);
