...
EspBoost* esp_boost_init(void) {
    ...
    // Send a probe command; Ghost ESP responds to "info\n"
    esp_send(boost, "info\n");
    furi_delay_ms(ESP_PROBE_DELAY_MS);

    // If UART acquired successfully on the expected GPIO channel,
    // an ESP32 board is physically connected. Mark as connected.
